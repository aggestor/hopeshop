...
const DataList : React.FC<Props> = ({onChange,className, type='text',value,placeholder,name, autoFill = false, children}) =>{
     return<div className="w-full relative group  h-fit">
        ...
        <div className="rounded-xl group-focus-within:flex hidden transition-all duration-500 flex-col absolute bg-white p-2 min-h-[200px] z-40 w-full top-16 border-black border-2 left-0">
            {children}
        </div>
     </div>
}
